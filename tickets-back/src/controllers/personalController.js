...
function obtenerPersonal(req, res) {
    if(connection){
        ...
        let sql = `SELECT * FROM PERSONAL WHERE personal_id = ${connection.escape(personal_id)}`;
        connection.query(sql, (err, personal) => {
            if(err){
                console.log(err);
            } else {
...
function crearPersonal(req, res){
    if(connection){
        ...
        if(personal.personal_nombre && personal.personal_nombre.length > 50){
            return res.status(400).send({error: true, mensaje: "La longitud del nombre debe ser máximo de 50 caracteres"});
        }
        if(personal.personal_apellidos && personal.personal_apellidos.length > 80){
            return res.status(400).send({error: true, mensaje: "La longitud del apellido debe ser máximo de 80 caracteres"});
        }
        if(personal.personal_telefono && personal.personal_telefono.length !== 10){
            return res.status(400).send({error: true, mensaje: "La longitud del telefono debe ser máximo de 10 caracteres"});
        }
