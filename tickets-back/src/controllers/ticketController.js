...
function obtenerTicket(req, res) {
    if (connection) {
        ...
        let sql = `SELECT * FROM TICKET WHERE ticket_id = ${connection.escape(ticket_id)}`;
...
function crearTicket(req, res) {
    if (connection) {
        ...
        if (ticket.ticket_descripcion && ticket.ticket_descripcion.length > 100) {
            return res.status(400).send({ error: true, mensaje: "La longitud de la descripción debe ser máximo de 100 caracteres" });
        }
        ...
        connection.query(sqlpersonal, (err, personal) => {
            ...
            if (personal === undefined || personal.length == 0) {
                return res.status(400).send({ error: true, mensaje: "El personal no existe" });
            } else {
